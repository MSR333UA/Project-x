 ...
 */
function wpw_cfu_pro_db_update() {
	
	global $post, $wpdb;
	
	$settings = get_option( 'wpw_cfu_settings', array() );
	
	if ( isset( $settings['db_version'] ) && $settings['db_version'] >= WPWHAM_CHECKOUT_FILES_UPLOAD_PRO_DBVERSION ) {
		// all good
		return;
	}
	...
	// Done
	$settings['db_version'] = WPWHAM_CHECKOUT_FILES_UPLOAD_PRO_DBVERSION;
	update_option( 'wpw_cfu_settings', $settings );
	
}
add_action( 'init', 'wpw_cfu_pro_db_update' );
