<?php
/*
Plugin Name: Checkout Files Upload for WooCommerce (Premium)
...
define( 'WPWHAM_CHECKOUT_FILES_UPLOAD_PRO_VERSION', '2.1.4' );
define( 'WPWHAM_CHECKOUT_FILES_UPLOAD_PRO_DBVERSION', '2' );
define( 'WPWHAM_CHECKOUT_FILES_UPLOAD_PRO_PATH', plugin_dir_path( __FILE__ ) );
define( 'WPWHAM_CHECKOUT_FILES_UPLOAD_PRO_FILE', __FILE__ );

/**
 * Update scripts
 */
require_once( WPWHAM_CHECKOUT_FILES_UPLOAD_PRO_PATH . 'includes/checkout-files-upload-woocommerce-update.php' );

require_once( WPWHAM_CHECKOUT_FILES_UPLOAD_PRO_PATH . 'updater/plugin-updater.php' ); 

register_deactivation_hook( __FILE__, array( 'Alg_WC_Checkout_Files_Upload', 'checkout_files_upload_deactivate' ) );
...
final class Alg_WC_Checkout_Files_Upload {
	...
	function __construct() {
		...
		// The filter
		add_filter( 'alg_wc_checkout_files_upload_option', array( $this, 'checkout_files_upload_option' ), PHP_INT_MAX, 3 );
	 ...
	 * @since   1.5.0
	 */
	public static function checkout_files_upload_deactivate() {
	
		wp_clear_scheduled_hook( 'wpwham_checkout_files_upload_updater' );
		$updater = Wpwham_Checkout_Files_Upload_Updater::get_instance();
	...
	public function enqueue_scripts() {
		...
		if (
			...
			wp_enqueue_script(
				...
				array( 'jquery' ),
				WPWHAM_CHECKOUT_FILES_UPLOAD_PRO_VERSION,
				false
			);
			wp_localize_script(
				'wpwham-checkout-files-upload-admin',
	...
	/**
	 * checkout_files_upload_option.
	 *
	 * @version 1.4.4
	 * @since   1.0.0
	 */
	function checkout_files_upload_option( $value, $type, $args = array() ) {
		switch ( $type ) {
			case 'settings':
				return '';
			case 'settings_total_files':
				return array( 'min' => 1 );
			case 'settings_total_files_button':
			...
			case 'total_number':
				return get_option( 'alg_checkout_files_upload_total_number', 1 );
		}
	}
	...
	function includes() {
		...
		$this->extensions->register( 'checkout-files-upload-for-woocommerce' );
	}
